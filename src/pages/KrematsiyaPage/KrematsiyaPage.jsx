...
import "./KrematsiyaPage.scss";

/**
 * Страница "Организация кремации в Шуе"
 * Полная организация кремации с описанием услуг, пакетами, этапами и FAQ
 */
const KrematsiyaPage = () => {
  const [faqJsonLd, setFaqJsonLd] = useState(null);

  // SEO данные для страницы - мемоизированы для оптимизации
  const seoData = useMemo(() => ({
    ...KREMATSIYA_SEO_DATA,
    canonical: KREMATSIYA_SEO_DATA.ogUrl,
    jsonLd: [KREMATSIYA_JSON_LD, faqJsonLd].filter(Boolean)
  }), [faqJsonLd]);
  ...
  return (
    <Global seo={seoData} pageClass="krematsiya-page">
      <Helmet>
      ...
      <main>
        ...
        <section className="krematsiya-services" aria-labelledby="services-title">
          <div className="container">
            <h2 id="services-title" className="krematsiya-services__title">
              {SERVICES_CONTENT.title}
            </h2>

            <p className="krematsiya-services__description">
              {SERVICES_CONTENT.description}
            </p>

            <div className="krematsiya-services__grid">
        ...
        {/* Стоимость организации кремации - таблица пакетов */}
        <section className="krematsiya-packages" aria-labelledby="packages-title">
          <div className="container">
            <h2 id="packages-title" className="krematsiya-packages__title">
              {PACKAGES_DATA.title}
            </h2>

            <p className="krematsiya-packages__description">
            ...
            <DataTable
              ...
              isColumnsEqual={true}
              enhancePrices={true}
              className="krematsiya-packages__table"
            />

            <p className="krematsiya-packages__note">
              {PACKAGES_DATA.note}
            </p>
          </div>
        </section>

        {/* Как проходит организация кремации - временная шкала */}
        <ServiceTimeline
